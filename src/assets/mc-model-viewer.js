...
export function ModelViewer(container, dimension = null, position = null, target = null) {
...
export function JsonModel(name, rawModel, texturesReference, clipUVs) {
    ...
    Object.keys(model.textures).forEach(function(key, index) {
      ...
      if (reference.name == textureName) {
        ...
        if (reference.hasOwnProperty('mcmeta')) {
          ...
          var imageBuffer = reference.texture
          ...
          ctx.drawImage(imageBuffer, 0, -i*width)
          // register texture
          textures[key] = canvas.toDataURL('image/png')
